...
/// Following SOLID: This class handles basic storage and defines the interface.
class Pair
{
protected:
    int first_;
...
public:
    ...
    /// @brief Virtual destructor is MANDATORY for base classes to prevent memory leaks
    virtual ~Pair() = default;

    // Field Accessors
    int getFirst() const { return first_; }
    int getSecond() const { return second_; }
    void setFirst(int f) { first_ = f; }
    void setSecond(int s) { second_ = s; }
    ...
    virtual PairType getType() const { return PairType::Base; }
};
